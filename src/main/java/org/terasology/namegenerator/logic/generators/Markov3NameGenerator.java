...
/**
 * Implementation of the {@link org.terasology.namegenerator.logic.generators.NameGenerator} interface, using Markov
 * chain model.
 * <p/>
 * The look-ahead for analysis and generation is three characters.
 * <p/>
 ...
 */
public class Markov3NameGenerator implements NameGenerator {
     ...
     */
    public Markov3NameGenerator(long seed, AssetUri assetUri) {
     ...
     */
    public Markov3NameGenerator(long seed, String simpleUri) {
        ...
        if (nameGenComp != null) {
            ...
            probabilities = new int[characters.size()][characters.size()][characters.size()][characters.size()];
     ...
     */
    public Markov3NameGenerator(long seed, List<String> sourceNames) {
    ...
    private void addStringToProbability(final String name) {
        ...
        char last3 = TERMINATOR;
        int index = 0;
        while (index < lowerName.length()) {
            if (characters.indexOf(lowerName.charAt(index)) != -1) {
                char current = lowerName.charAt(index);
                probabilities[characters.indexOf(last1)][characters.indexOf(last2)][characters.indexOf(last3)][characters.indexOf(current)]++;
                last1 = last2;
                last2 = last3;
                last3 = current;
                index++;
    ...
    /**
     * Chooses a random character from the probability matrix based on the previous three characters.
     ...
     * @param last2 the third character
     * @param last3 the last character
     * @return the next character based on the last two characters and probability matrix
     */
    private char nextCharByLast(char last1, char last2, char last3) {
        int total = 0;
        for (int i : probabilities[characters.indexOf(last1)][characters.indexOf(last2)][characters.indexOf(last3)]) {
            total += i;
        }
        ...
        int subTotal = 0;
        do {
            subTotal += probabilities[characters.indexOf(last1)][characters.indexOf(last2)][characters.indexOf(last3)][index++];
    ...
    public String nextName() {
        ...
        char last3 = TERMINATOR;
        do {
            char temp = nextCharByLast(last1, last2, last3);
            last1 = last2;
            last2 = last3;
            last3 = temp;
            if (last3 != TERMINATOR) {
                result += Character.toString(last3);
            }
        } while (last3 != TERMINATOR);
